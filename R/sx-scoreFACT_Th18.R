...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Th (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Th 
...
#' data.  As its default, \code{scoreFACT_Th18} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Th18}.
#' 
#' 
#' @param df A data frame with the FACT-Th items, appropriately-named.
...
#'   \item{ThS}{Thrombo-Cytopenia subscale}
#'   \item{FACT_Th_TOTAL}{FACT-Th Total Score (i.e., PWB+SWB+EWB+FWB+ThS)}
#'   \item{FACT_Th_TOI}{FACT-Th Trial Outcome Index (e.g., PWB+FWB+ThS)}
#' }
#' 
#' @references FACT-Th Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('An5', 'An7', 'Th1', 'Th2', 'Th3', 'Th4', 'Th5', 'Th6', 'Th7', 'Th8', 'Th9', 'HI7',
#'   'Th10', 'Th11', 'Th12', 'Th13', 'Th14', 'Th15')
...
#'                       'ThS', 'FACT_Th_TOTAL', 'FACT_Th_TOI')])
scoreFACT_Th18 <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("An5", "An7", "Th1", "Th2", "Th3", "Th4", "Th5", 
        "Th6", "Th7", "Th8", "Th9", "HI7", "Th10", "Th11", "Th12", "Th13", 
        "Th14", "Th15"))
    revNames <- toupper(c("Th1", "Th2", "Th3", "Th4", "Th5", "Th6", "Th7", 
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ThS_N, FACT_Th_TOTAL_N, ThS, 
            FACT_Th_TOTAL, FACT_Th_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ThS, FACT_Th_TOTAL, FACT_Th_TOI))
    }
    return(dfOut)
} 
